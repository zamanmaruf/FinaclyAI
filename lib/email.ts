...
export async function sendSignupNotificationEmail({
  ...
  try {
    const mailOptions = {
      from: `"Finacly AI Signup Notifications" <${process.env.EMAIL_USER}>`,
      to: 'finacly.ai.inc@gmail.com',
      subject: `🎉 New Early Access Signup: ${fullName} from ${companyName}`,
      html: `
        <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
          <div style="background: linear-gradient(135deg, #0A1128 0%, #1e293b 100%); padding: 40px; border-radius: 12px; color: white;">
            ...
            <div style="background: rgba(255, 255, 255, 0.1); padding: 20px; border-radius: 8px; margin: 20px 0;">
              <h3 style="color: #FFB800; margin-top: 0;">Company Details</h3>
            ...
            <p style="font-size: 12px; color: #64748b; text-align: center;">
              This notification was sent from the Finacly AI early access signup form.
            </p>
          </div>
        </div>
      `,
    ...
    await transporter.sendMail(mailOptions)
    console.log(`✅ Signup notification sent to finacly.ai.inc@gmail.com for ${email}`)
    return true
  } catch (error) {
